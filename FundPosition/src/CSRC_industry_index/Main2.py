...
def FundAvgProfit():
    ...
    for i in range(0, len(fundid)):
        Value = []
        print(fundid[i][0])
        sql = "SELECT id,value FROM fund.csrc_industry_index where indexid ='" + fundid[i][0] + "' order by datatime"
        cursor.execute(sql)
...
def Fundid():
    sql = "SELECT DISTINCT indexid FROM fund.csrc_industry_index;;"
    cursor.execute(sql)
    results = cursor.fetchall()
    return results

def MysqlDB(param):
    sql = "update fund.csrc_industry_index set profit=%s where id=%s"
